...
- **`Notes`** (M5) — permanent note records, single-table (PK `USER#<sub>` / SK `NOTE#<ulid>`). The note body Markdown lives in S3 (`bodyS3Key`); only metadata is in DynamoDB. Three item shapes share the table:
  ...
  - **GSI4 `ByRecipient`** (M7, projection ALL): `gsi4pk = USER#<recipientSub>`, `gsi4sk = SHARED_AT#<ISO-8601>` — list notes shared *with* a recipient, newest-first. On SHARE items only (`shareKeys`).
  - **GSI5 `ByDue`** (M8, projection ALL): `gsi5pk = USER#<sub>`, `gsi5sk = DUE#<ISO-8601 dueAt>` — list a user's due flashcards oldest-due-first. On CARD items only (`cardKeys`).
  - **GSI6 `StudySetsByUser`** (M13, projection ALL): `gsi6pk = USER#<sub>`, `gsi6sk = STUDYSET#<ulid>` — list a user's AI study sets newest-first (`ScanIndexForward: false`). On STUDYSET items only (`studySetKeys`). The M13 spec called this "GSI4"; renumbered because GSI4/GSI5 were already taken (see above).
  - **GSI7 `StudySetsByNote`** (M13, projection ALL): `gsi7pk = NOTE#<sourceNoteId>`, `gsi7sk = USER#<sub>#STUDYSET#<ulid>` — list study sets derived from a source note, scoped to the caller via `begins_with(gsi7sk, 'USER#<sub>#STUDYSET#')` so the shared GSI partition never leaks across users.
  - **GSI8 `ByQuizAttempt`** (M15, projection ALL): `gsi8pk = QUIZ#<quizId>`, `gsi8sk = GRADEDAT#<ISO-8601>` — list every attempt for a quiz, newest-first (`ScanIndexForward: false`; `GRADEDAT#<ISO>` sorts lexicographically). On ATTEMPT items only (`attemptKeys`, sk `ATTEMPT#<quizId>#<attemptId>`). The partition is keyed by quiz, NOT user, so the query returns all users' attempts — callers MUST filter by ownership (`pk === USER#<sub>`) or use the base-table `listAttemptsForUserQuizQuery`. quizId === the quiz's studySetId (M15 quizzes are `STUDYSET` items of `type: "quiz"`; the quiz body + answer key live at `storageKeys.studySetBody` and are never served raw — `GET /api/study/[id]/questions` strips the answer key, `GET /api/study/[id]/body` rejects quizzes).
  - **GSI9 `SourcesByUser`** (M20, projection ALL): `gsi9pk = USER#<sub>`, `gsi9sk = SOURCE#<ulid>` — list a user's uploaded document sources newest-first (`ScanIndexForward: false`, ULID lexicographic). On `SOURCE#` items only (`sourceKeys`, sk `SOURCE#<sourceId>`). The M20 spec called this "GSI7" but GSI7/GSI8 were already taken (StudySetsByNote / ByQuizAttempt), so it was renumbered to GSI9 — the real DynamoDB index name is `GSI9` (the friendly names here are doc aliases). `sourceKeys.countByUser` is the `Select: COUNT` variant for the per-user source cap (M20 guardrails). Source original files live in S3 at `storageKeys.sourceOriginal(sub, id, ext)` (`sources/users/<sub>/<id>.<ext>`); extracted Markdown at `storageKeys.sourceText(sub, id)`.
...
- **Dependabot PRs never deploy a `pr-<N>` stage.** Dependabot-triggered runs read from the *Dependabot* secret store rather than the Actions secret store, so `AWS_ROLE_ARN` / `CLOUDFLARE_API_TOKEN` come back empty and the OIDC step fails with `Could not load credentials from any providers` — which used to fail every dependency bump for a reason unrelated to the bump. `deploy.yml`'s deploy gate now excludes `dependabot/*` head refs (and `teardown.yml` skips them to match). All the quality gates — lint, typecheck, unit, integration, marketing E2E — still run on Dependabot PRs and still gate the merge; only the SST deploy steps are suppressed.
